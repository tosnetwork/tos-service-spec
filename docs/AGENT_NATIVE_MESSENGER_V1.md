...
Progress snapshot (2026-08-21, audited through `tos-messenger` `a2cd605`,
...
Weighted implementation completion is now **about 88% (bounded estimate
86–90%)**, up from the prior approximately 87% audit. The cumulative increase is the
...
because no durable channel store or Overlay path exists, this is not a
user-visible product increment. Product readiness remains
...
completeness, moderation power, or payment state. Messenger `a2cd605` therefore
starts above transport with `pkg/publicchannel`:
...
- a complete bounded Event set has one sorted-ID history commitment independent
  ...
  only a claim until fetched Events reproduce it through full verification.
...
**Status: 🟡 Overlay primitives and the route-neutral signed authority/event/
history candidate are implemented; durable storage, transport and product
remain open.**
