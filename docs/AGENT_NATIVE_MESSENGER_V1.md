...
Progress snapshot (2026-08-21, audited through `tos-messenger` `a44aee9` and
OpenFox `aa6fe1f7`): the component inventory is **5/20 ✅**, with
...
Weighted implementation completion is now **about 72% (bounded estimate
70–75%)**, up from the prior approximately 71% audit. The increase is the
...
application handshake and owner-private local history; it is not
a status promotion of an entire component row. Product readiness is
approximately **56%**: local users
...
through restart. This closes the
...
| MSG-014 | OpenFox channel and local IPC | `openfox` / `tos-messenger` | 🟡 encrypted three-OpenFox local IPC has separate MLS state owners, an opaque Relay, durable exact retries, tamper refusal and full restart. OpenFox `ed1fcc75` additionally runs the three channel owners as separately supervised OS processes with private durable transcripts/control sockets, stable caller IDs, bounded catch-up replies and fail-visible errors; restart replay retained one opening and exactly two Event-bound peer replies without Relay plaintext. The production adapter claims daemon events, independently checks Event ID and canonical `text`/`room.message`/`room.moderation`, binds room identity/epoch, publishes typed group input, and durably retracts moderated applied history and authority without sending controls to the model. OpenFox `aa6fe1f7` also holds ordinary leases until an exact Event-ID/content/provenance session record is fsynced, makes crash replay model-idempotent, refuses substitution/non-durable history, keeps busy input outside volatile steering, and protects local history with owner-only modes. It submits authenticated replies under operator-bound routes to daemon-owned construction, with durable restart-stable Event IDs and substitution refusal. Selected transport binding and real-network evidence remain |
...
Work-package progress (2026-08-21, audited through `tos-messenger` `a44aee9` and OpenFox `aa6fe1f7`): **6/32 ✅**, 19/32 🟡, 4/32 ⬜,
