...
Progress snapshot (2026-08-21, audited through `tos-messenger` `e3ea408`,
...
than live independent Storage evidence.
Product readiness remains
approximately **69%**: local users can exercise real encrypted group behaviour
and the public-channel path now has a runnable native discovery/transport
daemon plus verified Storage Bag publication/catch-up adapters; measured route
selection, independently operated deployment and wire-freeze evidence remain
the dominant gates.
...
- a single-writer catch-up consumer invokes the stock CLI `add-by-hash` and
  ...
  injected downloader; the CLI executable itself remains a process fixture.
...
Still open are measured limit calibration, live independently operated
...
publication/hints, and bounded stock CLI download/catch-up are implemented;
