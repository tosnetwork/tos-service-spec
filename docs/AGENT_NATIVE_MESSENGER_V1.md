...
Progress snapshot (2026-08-21, audited through `tos-messenger` `7c6e43d`,
...
Weighted implementation completion is now **about 90% (bounded estimate
88–92%)**, up from the prior approximately 89% audit. The cumulative increase is the
...
increment. `4eb6c9f` additionally freezes the candidate's signing and
convergence bytes plus a five-case adversarial corpus without claiming an
independent consumer. `7c6e43d` then closes the route-neutral recursive history
fetch/verification seam while deliberately leaving the actual Overlay/RLDP/
Sites carrier and its peer policy unselected. Product readiness remains
...
- bounded fetch starts at untrusted tips and follows exact causal IDs. Each
  ...
  set reproducing the exact head can enter the durable ledger.
