...
`tos-messenger` main `463c43f`, OpenFox `f4087f63`, and `tos-ai` `a9928de`): the component inventory is **5/20 ✅**, with
...
93–96%)**; the authenticated attachment service and reference content-admission closures stay inside the same band and promote no
...
content-admission boundary implemented; independent-network, OpenFox and
production scanner integration pending.**
...
`artifact.encrypted` Events emit schema v2; immutable v1 history is decoded
only when the Event explicitly declares v1, while new legacy-schema emission
is refused.
...
not yet fetch or consume `artifact.encrypted`. Independently operated public
...
evidence, OpenFox application wiring and commercial terms remain open.
...
Work-package progress (2026-08-21, audited through TOS Core main `0aac896`, `tos-messenger` main `463c43f`, OpenFox `f4087f63`, and `tos-ai` `a9928de`): **6/32 ✅**, 19/32 🟡, 4/32 ⬜,
