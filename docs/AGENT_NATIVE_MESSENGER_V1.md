...
`tos-messenger` main `13b5ccc`, OpenFox main `5eab6cf8`,
...
OpenFox main `aa02f93e` and Messenger main `a9a1051` close the local
machine-verification gap for that deployment: replay is now read-only-gated by
the durable request/Event binding, complete transcripts and Relay bytes are
compared, and exact running artifacts are pinned. This strengthens MSG-014 but
does not supply an independent operator, selected public route, independent
OpenMLS review or second implementation, so the rounded estimate and counts
remain unchanged.
...
the percentage and counts remain unchanged. Messenger main `ecdcedb` then
closes the local real-engine acceptance gap: all official database snapshots,
matching external signatures and the CVD root certificate are immutable and
digest-bound; `--fips-limits` prevents legacy-validation fallback; and three
ClamAV 1.5.3/current-daily sandbox rounds pass clean, EICAR, corrupt-signature
and corrupt-certificate cases. MSG-013 stays partial because this proves the
supply-chain path, not representative malware coverage. The remaining
local evidence-format gap is closed by Messenger main `8913b23`: a strict
externally signed corpus manifest and separately signed runner report bind the
approved samples, exact admission policy, clean source revision, scanner
resources and per-sample outcomes. This does not promote MSG-013 because no
qualifying externally approved representative corpus has yet produced such a
report. Messenger `6e91aed`, merged as main `48072c6`, closes the route-neutral
Messenger half of MSG-016/MSG-017: A2A and MCP carriage profiles are exact,
their Events can never fall through to model text, separate private consumers
receive the complete canonical Event under durable retry/completion, and
cross-profile/socket substitution fails closed. The rows remain 🟡 and the
rounded estimate does not move. `tos-ai` `551b64f`, merged as main `96bf0ef`,
now supplies the independently verifying, profile-separated Unix consumers,
strict official A2A/MCP mapping, real Messenger-client compatibility, shared
Gate entry and result-before-202 contract. At that checkpoint, production
composition, selected live transport and independent operation remained absent.
`tos-ai` `14c7888`, merged as main `526fc6e`, adds the Event-ID-idempotent,
crash-safe result outbox. Messenger `ecf4365`, merged as main `74e7cc2`, then
adds result-only daemon composition while retaining identity, network, clock,
schema and Event-ID authority; `tos-ai` `311bcab`, merged as main `7cd3e48`,
drains the outbox through exact sender-to-session/Endpoint routes and records
completion only after canonical queueing. The real Messenger journal,
runtime-socket and client integration proves the exact MCP result Event and no
replay after both journals restart. `tos-ai`
`bcf5846`, merged as main `a1126e4`, closes that production-composition gap:
one strict operator document now assembles the profile-separated consumers,
quorum finalized-state Gate, shared frozen V1 container runtime, persistent
artifact locator, outbound result publisher and hardened systemd unit. Exact
sender coverage is mandatory, duplicate JSON authority is refused, and inbound
MCP results commit to a distinct terminal journal so they cannot enter the
outbound publisher. Selected live transport and independent operation remain
...
evidence, a representative approved hostile-corpus run for the pinned ClamAV
release, and commercial storage remain outside that claim. The
...
calibration.
Messenger main `8c26720` closes the next locally executable calibration defect.
A valid 65,536-Event linear history can reveal only one new causal parent per
response, so the old 1,024-fetch native ceiling deterministically truncated
that valid shape; the server also copied and scanned the full history for every
exact-ID request. The native request budget now reaches the Event bound while
independent byte, unavailable-result and attempt-time ceilings remain, and an
immutable ID-to-position index makes serving one requested Event independent
of total history size. Same-host strict wire/verification benchmarks cover
1/8/32 concurrent peers; exact-ID lookup remained about 0.5 microseconds at
both 1,024 and 65,536 Events. This narrows the gap to representative-device,
public-network concurrent calibration and independent operation; it does not
freeze a route before M0-R.
Messenger main `9b91e1b` closes the duplicate-work defect exposed by that
concurrent calibration. `NativeNode` now single-flights synchronization for an
exact canonical Head across authenticated peers, permits at most one active
Head per peer, and bounds the pending public-data queue to 256 candidates.
Failure promotes exactly one still-connected peer advertising the same Head;
success removes redundant waiters, stale completions are ignored, and distinct
ready Heads may still progress independently. The key is the complete strict
Head encoding rather than only `history_digest`, so a same-digest tip
substitution cannot suppress a canonical candidate. Concurrency, disconnect,
failure, replay, queue-overflow, substitution and shutdown cases pass focused
...
Messenger `6949c23`, merged as main `f0ee6df`, closes the remaining local
runnable-group removal gap without pretending to supply production room
authority. Fresh MLS lab state v2 separates the immutable untrusted Relay
delivery roster from the active cryptographic roster and fixes the bootstrap
creator as this lab's sole membership controller; existing v1 state remains
message/restart compatible but cannot be guessed into v2 authority. One
removal call first creates a genuine OpenMLS Remove Commit, then seals the
removed Agent identity plus that exact Commit digest under the old epoch. Every
peer verifies the private binding before applying the Commit, fsyncs the new
epoch/cursor before opening a later Relay item, suppresses the control from the
Agent transcript, and returns a restart-stable result for the caller's exact
client ID. The real-driver test proves non-controller and changed-target
refusal, valid-Commit splice refusal, removal-frame identity opacity, full
proxy restart, a two-member active view, and no-future secrecy when the Relay
continues delivering ciphertext to the removed process. It passed ten focused
runs, three focused race runs, complete local `make verify`, and all fifteen
remote verify/vector/cross-build/fuzz jobs. This strengthens MSG-014/MSG-021
local evidence but does not provide finalized production membership authority,
an independently operated Relay, independent review or a second implementation;
both rows therefore remain 🟡.

Messenger `d0a8923`, merged as main `bb2681a`, and OpenFox `896ff19d`,
merged as main `c5746b2a`, define removal as an explicit terminal membership
state rather than a transport error. If one Relay page contains the authentic
old-epoch removal control followed by future ciphertext, the proxy fsyncs the
new OpenMLS epoch and Relay cursor, returns only pre-removal application
messages, and stops exactly at the control boundary. Later room replay and
polling return `410 Gone`; private `/livez` remains `200` and binds the original
room label/member set, room ID, final MLS epoch and `active_member: false`.
The test then obtains the exact later ciphertext directly from the immutable
Relay delivery roster and feeds it to the removed Agent's exact persisted
OpenMLS state; decryption is required to fail, so expected no-future secrecy is
not mislabeled as Relay corruption.

OpenFox recognizes that `410` without a retry loop, deletes the retired room
and cursor durably, and refuses subsequent sends locally. After process
restart it validates the operator's exact configured label/member set against
the private proxy health, recovers the terminal room ID, keeps the control
process healthy with `active_member: false`, and returns `410` from `/v1/send`
before entering the channel. Changed configuration or an invisible-but-active
membership still fails startup. Messenger passed ten focused removal runs,
three focused race runs, complete local `make verify`, and all fifteen remote
verify/vector/cross-build/fuzz jobs. OpenFox passed twenty focused runs, three
focused race runs, complete tagged tests and vet; an unrelated dynamic-port
test that collided once while both repositories' network suites ran in
parallel passed 50/50 alone and the subsequent serial full suite passed. These
are local correctness and cryptographic exclusion evidence, not a selected
public route, independent operation/review, or second implementation; the two
component rows therefore remain 🟡.
...
After both terminal-state changes merged, the acceptance host rebuilt the
Relay and MLS proxy from exact Messenger main
`bb2681ae3f314162f8c924d410c4d882eaefaa50` and the Agent from exact OpenFox
main `c5746b2a416ff351325bcc78c42c2f557b770721`; all three Go binaries report
`vcs.modified=false`. Their SHA-256 values are respectively
`b704fcf4165323d185a9a17a9458ab389e50ab9a0a04f845c3b21f6a84aa0a82`,
`d7a725b075f3b1b676104bfcfd3db2c8187408b037f35f7eb01c93e1803ad44a`,
and `538e20d78b0245cf7eb140e20e747782533d51bfef83e08d75ec78161e768edf`.
All seven services restarted `active/running` with `NRestarts=0`; Alice, Bob
and Carol reported the same deployed room with `active_member: true`. Exact
historical replay again returned opening
`msg_0234085f7cfa90e9a0f382f84c746704a9c82d0097a9d1f190376aa40b0dc776`,
each transcript remained at 84 entries with exactly one opening and the same
one Bob/Carol reply, the Relay file remained mode `0600` with none of their
plaintext, and the post-restart warning journal was empty.

A separate disposable fresh-v2 real-process run then created room
`room_75b9b943d47c2735e94a4a5f528ab22298621f96b5401d96c1992777b7f6ffd4`
at epoch 2, started one Relay, three private proxies and three OpenFox Agent
processes, and removed Carol with control Event
`msg_f8f4df5a7dd02f1b66abf936f40bd2e2f9c01fbbfff8340eefbf138ca34ca55e`.
Carol reached epoch 3 and `active_member: false`; after her Agent process was
stopped and started again against the same private state, it remained alive,
reported the same terminal room ID and returned `410` for a new send. The
...
After signed-corpus tooling merged as Messenger main
`8913b23f38aabb5073f6cf3d6a01f941e927b6a9`, the host rebuilt the Relay and
MLS proxy with `vcs.modified=false`, retained OpenFox main
`b2e140a28078b0c838ab38e726973427b931ab49`, and retained the same OpenMLS
Driver. Installed SHA-256 values are respectively
`b9e72d47a843bb2ec7accb930ece212144e93d151bba9c43894193c569a6e6f6`,
`5f0997bb1600f5db82dba48eccf729b69ceff79dc9d74dce6bef9e490ebbd33e`,
`371eb8c65e3253ba69aa2fa8407e31799e72afc3d685b2fcbe71eae4b5ad54e4`,
and `7a0ab29e79042b6d2028d15de9d17bdc8312ce21ba70c5e6b5dd6bd02ad5756f`.

Alice then sent opening
`msg_6cdf352d51c64f5577e3d38d9bb5cd3696b04511038bfd295083293ab69b34fe`.
Bob and Carol each produced exactly one `openfox-agent-loop` reply,
`msg_51db663d34386a6b9ea33fb3aa66b8c23ef150a7e2cfa86651e3f0b9b788f9d1`
and `msg_60f1f77b22df26c0ce5ba446e6997e7f49cc36beb4fd5843005d02d1f70b7a1f`,
both bound to the opening as `reply_to_event_id`; all transcripts advanced
from 96 to 99. Exact replay before and after a full seven-service restart
returned the same opening and left all counts at 99. Every service remained
`active/running` with `NRestarts=0`; the mode-`0600` Relay contained none of
the new plaintext and all seven warning-or-higher journals were empty. This is
exact-main, same-host OpenFox group-chat evidence, not independent-network or
hostile-corpus approval evidence.

After public-channel concurrent calibration merged as Messenger main
`8c267201f23e8e8b497db37fa85fa6cddd38f66c`, the installed Relay and MLS proxy
...
`cd3e0aa1d8ef347360bdbdbf2962ed0bec2447bb86202973b38692abcd3076b8`,
`f3dfdc111dc76af5bc817e8a51da032c0a526764c809e1464ef24a8edc255b7e`,
`371eb8c65e3253ba69aa2fa8407e31799e72afc3d685b2fcbe71eae4b5ad54e4`,
and `7a0ab29e79042b6d2028d15de9d17bdc8312ce21ba70c5e6b5dd6bd02ad5756f`.

Alice sent opening
`msg_24616a0eb42038e805839aeba851568f9ccd7f9b662d55a11485c0e420f3a331`;
Bob and Carol produced exactly one `openfox-agent-loop` reply each,
`msg_89f2370c0e9aec2c4703f2a75d5be672fbc3f758e747787eea85e1754868f273`
and `msg_7b1eb3fbbf2eda6dcfc24ded7a4af3d0502a86cab3196c083bdf32d313ab96a5`,
both bound to the opening. All transcripts advanced from 99 to 102. Exact
online replay and replay after a seven-service restart returned the same
opening and left all counts at 102. Every unit remained `active/running` with
`NRestarts=0`; the mode-`0600` Relay held no new plaintext and all seven
warning-or-higher journals were empty. This re-proves the exact-main OpenFox
group-chat closure but remains same-host evidence.

After same-Head synchronization single-flight merged as Messenger main
`9b91e1b448afffac5d260d7cf98a2b108708f3c2`, the installed Relay and MLS proxy
reported that exact revision with `vcs.modified=false`; OpenFox remained exact
main `b2e140a28078b0c838ab38e726973427b931ab49`. Installed Relay, proxy,
OpenMLS Driver and OpenFox SHA-256 values were respectively
`0def646489646211cffc22e1bf876ff9c91efb8236a41dc304c5ea1959da5f7f`,
`da66c76228a8831c2edfc49f88cb1a2d6235da03acc0be199c04d1ecf95e69cb`,
`371eb8c65e3253ba69aa2fa8407e31799e72afc3d685b2fcbe71eae4b5ad54e4`,
and `7a0ab29e79042b6d2028d15de9d17bdc8312ce21ba70c5e6b5dd6bd02ad5756f`.

Alice sent opening
`msg_d646682fdbeb473e7e5b17cd0f032e4faf9c81576ca1f22cb22c1455eb02c321`;
Bob and Carol produced exactly one `openfox-agent-loop` reply each,
`msg_f2a4389b04b609f68e77c46f72e3acbb4fa631c422c3e00aae8ded9eecaf2ec0`
and `msg_fb0c025f2e531b0186368e99a35af41bc872aa67db4dcfa273ae1c4632a90a18`,
both bound to the opening. All three transcripts advanced from 102 to 105.
Exact online replay and replay after a full seven-service restart returned the
same opening and left every count at 105. Every unit remained
...
production DHT/HTTPS refresh retained by daemon config v9; the approved
...
committed. The production finalized-state adapter, strict bounded private Relay
listener/client, amplification ceiling, durable nonce claims and exact
multi-Relay ACK thresholds are now implemented. Post-M0-R public transport
binding, push-hint operations and independent-operator failover evidence remain
open.
...
`3c6a329`. Daemon config v9 retains the v5 policy contract. It explicitly states
...
Processing must stop at the durably authenticated self-removal boundary rather
than attempt later-epoch ciphertext with old state. Subsequent application
polling and room replay must expose a stable terminal membership result, while
operator health remains available with the final room identity and epoch.
OpenFox must retire the room from polling and sending without retrying that
terminal result as an outage; after restart the Agent process remains healthy
and reports inactive membership instead of silently resurrecting the room or
exiting as though the Relay had failed.
...
authority gap. Daemon config v9 requires one explicit Event-bounded plaintext
...
Messenger main `5f5c8be` adds an optional hard Linux scanner profile rather
than silently upgrading the legacy `prlimit` claim. The daemon rehashes and
private-stages the exact pinned `/usr/bin/systemd-run` inode, requires a clean
absolute mode-private `XDG_RUNTIME_DIR`, and starts each scan in an
unpredictably named transient service. The unit fixes memory accounting and
`MemoryMax`, `MemorySwapMax=0`, task accounting and `TasksMax`, `LimitCORE=0`,
`NoNewPrivileges`, a scanner-timeout `RuntimeMaxSec`, and whole-control-group
termination. Bounds, unsafe runtime authority and launcher substitution fail
closed. The scanner still receives an empty environment inside the networkless
bubblewrap sandbox, and cgroupfs is not mounted there.

A live test proves a normal scanner observes `/tos-attachment-scan-...` and a
zero core limit. A second helper allocates and touches 512 MiB under a 256 MiB
unit cap; the cgroup OOM-kills it and `OpenForAgent` releases no plaintext.
That test passed twenty repeated focused runs and three race runs; affected
packages, complete local `make verify`, and all fifteen remote
verify/vector/cross-build/fuzz jobs are green. This is same-host implementation
and operating-system evidence, not an independent hard-isolation review. Hosts
...
Messenger main `ecdcedb` advances that adapter to an exercised real-engine
boundary. It requires the pinned ClamScan engine, sorted official
`main`/`daily` CVD or CLD snapshots plus `bytecode`, one matching versioned
external `.cvd.sign` per database, and the pinned CVD root certificate. It
invokes only those databases with `--official-db-only=yes`, forces external
signature verification with `--fips-limits`, uses the exact authenticated
input as `--max-filesize`, and permits only a fixed 1 MiB internal
scan-accounting allowance. Exit 0 is allow, exit 1 is deny, and every other
engine outcome is failure. Local response v5 carries all eight exact resources,
and OpenFox main `b2e140a2` independently validates canonical names, ordering,
count and SHA-256 digests before publishing admitted text to the Agent bus.

The 2026-08-22 acceptance used ClamAV 1.5.3 with `daily` 28099 dated
2026-08-21, `main` 63 and `bytecode` 339. The immutable engine/database/
signature/certificate snapshot included SHA-256 `a76f6c89f50049416f5498d24bad41443ac81cd2a4afd338f0dd84295f2eb40e`
for `clamscan`, `e35bbe16cc365a41222ef9223c7bf22cc566c9c6bf1d721bfe435d9cd8f46539`
for `daily.cvd`, `0b2182d229f46981ec8f535382222f7c9dfdd656b250ad47988b910a8d302365`
for `main.cvd`, `6d4aa01f219e988060fc419f495d07f27e0cdf1a2cccc065971da922c76f7ffb`
for `bytecode.cvd`,
`b35347aa1b2cc6e19e785918390fcdf197fe5a7a6f207ae644748831de1768c3`
for `daily-28099.cvd.sign`,
`c030a2235e0ac8b53f95bc6f79563c2b3ddfdcf4fd1d722634578a79530898e2`
for `main-63.cvd.sign`,
`94eb76eff8cf3ab90ec3a67992727e84f9be257f294d72504228e89fa566592f`
for `bytecode-339.cvd.sign`, and
`60200e4b4b1b1b7257bb4550487aeb142034341f3300a8526e77a3134a940e9d`
for the certificate. Three consecutive exact outer-sandbox runs admitted inert
text, denied a runtime-constructed EICAR input, and failed closed when either
the daily external signature or root certificate was corrupted. Focused and
race suites, Messenger's full `make verify`, OpenFox's full `make check`, and
all fifteen Messenger remote jobs pass. This establishes a supported/current
real-engine and signature-supply-chain path; EICAR remains only a plumbing
assertion and is not representative malware coverage.

Messenger main `8913b23` supplies the missing evidence plumbing without
claiming the missing approval. `tos-attachment-corpus` creates mode-`0600`
Ed25519 keys, signs a strict externally authored corpus manifest, runs every
declared sample through the real `Seal` → `OpenForAgent` boundary, and signs a
separate report. The report commits the raw manifest and admission-policy
digests, exact clean VCS revision, Go toolchain, all eight scanner resources,
and each expected/observed allow-or-deny outcome. A completed scanner deny is
structurally distinct from timeout, resource, engine and infrastructure
failure; sample, policy, key, signature, resource, file-inode and output-path
substitution tests fail closed. PR #21 passed all fifteen remote jobs.

A deliberately self-operated plumbing run used distinct local approver and
runner public keys
...
Its manifest, admission policy and report SHA-256 values were
...
and `255eae67d927ce49841cda3ce479f180691935c6a854ca58e36c66dd52ecbc32`.
It admitted one inert control and denied one EICAR control; removing a scanner
resource aborted without a report, and runner-key substitution failed
verification. Its scope is explicitly `local-plumbing-only-2026`: neither the
key separation nor these two controls establish organizational independence,
external approval or representative hostile coverage.
...
a qualifying externally approved representative hostile-corpus report for the pinned ClamAV release,
independent
...
delivery. Messenger `13b5ccc` additionally supplies a deployable
Descriptor-bound `https-bootstrap` fallback: public exact-path TLS, public-DNS
pinning, no proxy/redirect, strict ciphertext frames, and exact
Endpoint-signed durable acknowledgements. It passes a real-TLS two-daemon local
acceptance but does not claim that M0-R selected the final native route. The daemon-owned
...
Phases C and D of the linked economy plan are locally implemented by OpenFox
`8d52b817`, `42268bd3`, `ab356fc1`, `129ec08d`, and `d3a64104`, plus protocol
`5248a15`, `284fc3a`, and `3809bb7`: observe-only finalized opportunity
discovery, exact policy-gated Quote/purchase projection, at-most-once
...
`3a82ae94`, `2bbb9434`, `13eb5ba8`, `95a25e70`, and `5eab6cf8`: two independently deployable real AgentLoop
...
gap. Daemon config v9 optionally assembles the concrete escrow-backed Quote
...
### 19.3 Physical AI safety — ✅ route-neutral local profile implemented
...
Messenger main `083c202` and OpenFox main `5a5624eb` implement that boundary.
Raw I2C, SPI, and serial tools are classified as the distinct `physical-io`
effect rather than an ordinary tool call. OpenFox registers a hardware tool
only when its local configuration names one exact `cap_` identifier and a
closed operation allow-list. The registry refuses missing or malformed
Capabilities, disallowed operations, and missing action authorization before
calling the tool. Both reads and writes remain physical effects: a read can
clear a register, expose a sensor, or change device timing and therefore does
not inherit a weaker default.

Every physical invocation requires a one-shot owner decision regardless of the
...
Focused physical-I/O tests passed twenty repeated runs and three race runs in
both repositories. Messenger's complete `make verify` passed twice locally and
all fifteen PR checks passed; OpenFox passed its available Go, tagged,
nested-module, Web, docs, build, and focused race gates. The host lacked the
optional `golangci-lint` executable, so this evidence does not claim a literal
successful `make check` invocation. Deployment-specific device profiles,
real-time interlocks, hazardous-operation analysis, hardware certification,
and operation on real equipment remain operator and external-safety evidence;
Messenger is neither a controller nor an interlock.
...
route decision. Daemon config v9 retains the v6 provider socket and bounded timeout;
...
| MSG-003 | Contact Descriptor, inbox-policy digest, and DHT locator | `tos-messenger` / future messaging spec | 🟡 strict schema/binding, per-Agent committed policy retrieval, explicit finalized delegation/policy bootstrap, daemon config v9 lifecycle wiring, production native DHT lookup/publication, verified bootstrap nodes, bare/prefixed network boundary, bounded SSRF-resistant HTTPS retrieval, durable admission, vectors, substitution and scheduled-revocation tests implemented; live independently operated multi-node evidence missing |
| MSG-004 | Bounded local TOS network adapter | `tos` / `tos-messenger` | 🟡 native DHT, ADNL, RLDP, Overlay, Storage and reachability adapters already expose bounded process/library seams with strict identity rebinding, time/byte/work ceilings and local real-process tests. The remaining one-to-one adapter is deliberately not selected: M0-R must first produce the accepted direct/tunnel/Relay ordering, after which this row can bind only that measured route and fallback policy. Implementing a preferred message adapter before that evidence would silently make the route decision this work package is forbidden to make |
...
| MSG-008 | Direct ADNL and RLDP integration | `tos-messenger` | 🟡 the native ADNL/RLDP carriers, exact peer-key rebinding, bounded request accounting, maximum-size ADNL echo and segmented RLDP loss/recovery paths run locally in M0-R and public-channel tests. Direct one-to-one Messenger delivery is intentionally absent until M0-R accepts a route strategy; it is not an independently executable pre-study task, and local same-host success cannot promote this row |
| MSG-009 | HTTPS bootstrap/fallback adapter | `tos-messenger` | 🟡 Messenger `13b5ccc` implements bounded Descriptor-bound HTTPS delivery in addition to discovery: public port-443 exact-path URLs only, public-only DNS pinning, TLS with no proxies/redirects, strict E2EE copy frames and exact finalized-Endpoint-signed durable acknowledgements. Real-TLS two-daemon local acceptance and full verification pass; independently operated public-network evidence and the final M0-R route selection remain open |
...
| MSG-013 | Encrypted attachment profile | `tos-messenger` | 🟡 cryptographic chunk/manifest/E2EE-reference core plus authenticated remote storage are implemented in Messenger main `026b987`: exact per-manifest grants; independent Endpoint/capability/storage keys; finalized delegation recheck per operation; body-bound one-use requests; durable nonce, monotonic-clock and generation defenses; bounded resumable multi-frame upload/fetch/delete; signed StoredAck/DeleteAck; periodic expiry/stale-stage GC; exact HTTPS locator and public-only DNS/SSRF policy. Main `463c43f` adds fail-closed `OpenForAgent` with sealed `memfd`, private-staged SHA-256-pinned scanner/bubblewrap/prlimit inodes, networkless/capability-dropped sandboxing, bounded all-must-allow verdicts and a parser-free UTF-8 inspector. Messenger `96a30ee`, merged as main `52f367c`, advances `artifact.encrypted` to v3 and reserves daemon-owned recipient admission. Messenger `13a4f48`, merged as main `4833a2c`, advances local request/response to v5/v3 and adds daemon-owned outbound streaming: ciphertext-only restart state, exact plaintext-digest commitment, external finalized Endpoint signing, distinct upload/fetch keys, resumable one-chunk storage calls, verified StoredAck-before-queue, prepared-only crash recovery, exact completed retry and post-queue secret cleanup. Messenger main `5f5c8be` adds optional pinned-systemd per-scan cgroups with hard memory/zero-swap/task/no-core/runtime/whole-group-kill policy, fail-closed authority/bounds/launcher handling, live cgroup observation and a 512 MiB-under-256 MiB OOM refusal test. Messenger main `ecdcedb` requires immutable engine/main/daily/bytecode/external-signature/CVD-certificate resources, forces external verification, and passes three real ClamAV 1.5.3/current-daily clean/EICAR/corrupt-signature/corrupt-certificate sandbox rounds. Messenger main `8913b23` adds fixed-authority signed corpus manifests and signed execution reports over the exact clean build, raw policy, scanner resources, samples and structural outcomes; local controls prove the plumbing while explicitly not claiming corpus approval. Real authenticated-store, maximum-shape, sandbox/tamper, authority substitution, key/launcher/sample substitution, repeated live cgroup, race/fuzz and all fifteen remote jobs pass. Still pending: independently operated public TLS/interrupted-transfer and audited retention evidence, a qualifying externally approved representative hostile-corpus report, independent hard-isolation and host-wide swap/crash-policy review, and commercial terms. The reference inspector is not a general malware or prompt-injection defense; DeleteAck is not erasure proof |
| MSG-014 | OpenFox channel and local IPC | `openfox` / `tos-messenger` | 🟡 encrypted three-OpenFox local IPC has separate MLS state owners, an opaque Relay, durable exact retries, tamper refusal and full restart. OpenFox `ed1fcc75` runs the three channel owners as separately supervised OS processes; `c47a98e0` further runs a real AgentLoop in each process with private durable workspaces, delayed cursor application, stable Event-bound replies and model-idempotent completed replay. A deployed round retained one opening and exactly two `openfox-agent-loop` peer replies after Alice restart without Relay plaintext. Messenger `6949c23`, merged as main `f0ee6df`, adds fresh-v2 lab membership removal: controller-only exact retry, private target→Commit-digest binding, fsynced epoch transition, control-free Agent transcript, active-room projection and removed-member no-future secrecy even under continued Relay delivery; v1 deployed state remains message/restart compatible. The production adapter claims daemon events, independently checks Event ID and canonical `text`/`room.message`/`room.moderation`, binds room identity/epoch, publishes typed group input, and durably retracts moderated applied history and authority without sending controls to the model. OpenFox `aa6fe1f7` and `7fd3ac11` hold leases through exact durable session application and preserve authenticated origin through the real AgentLoop response. OpenFox `9e241774` delegates route-free proactive send and authenticated direct reply authority to daemon operations. OpenFox `3a82ae94` adds a production-channel AgentLoop process with private durable transcript/control boundaries; `2bbb9434` adds restart-applied reply deduplication, per-run evidence identity, a strict two-transcript verifier and public-TLS two-host runbook; `5eab6cf8` adds canonical externally signed Ed25519 operator evidence binding transcripts, AgentIDs, public endpoints, network/genesis, commits, binary/configuration digests and bounded run intervals. OpenFox `cb8080d7`, merged as main `ac2475a3`, consumes current Event v2, drains only daemon-admitted attachments and independently rehashes body/scanner evidence. OpenFox `69dce589`, merged as main `40554b6c`, adds outbound `MediaSender`: exact registered file descriptors, stable route/origin/reply/media/digest idempotency, sequential 1 MiB resume, shared-caption causality and canonical daemon Event results, while every storage/key/network/Event authority stays in Messenger. Multi-chunk, retry, substitution, focused race and full test/vet/docs/build pass. Independent real-network/operator evidence remains |
| MSG-015 | Context firewall and approval policy | `openfox` / `tos-messenger` | ✅ policy/ceilings, authenticated owner queue and offline-signing CLI, crash-safe one-shot grants, durable runtime provenance, production authenticated ingress, classified pre-execution tools, bounded owner wait, mandatory exact-term native-buyer custody/key-use wrapping, post-funding finalized-Quote verification before dispatch, and an exact local-Capability + closed-operation + one-shot-owner gate for every `physical-io` invocation are implemented and tested |
| MSG-016 | A2A event bridge | `tos-messenger` / `tos-ai` | 🟡 `tos-ai` owns the A2A execution adapter. Messenger `6e91aed`, merged as main `48072c6`, fixes the carriage profile to protocol `a2a`, permanently excludes `a2a.message` from the general model/runtime inbox, and hands the complete canonical Event v2 to a fixed bounded proxy-free owner-private Unix endpoint under a kind-specific durable application lease. Consumer failure retries after lease expiry and successful completion survives restart without redelivery. `tos-ai` `551b64f`, merged as main `96bf0ef`, adds the paired mode-`0600` Unix service: it independently recomputes canonical Event v2, enforces exact network/sender/conversation policy and carriage version, maps the official A2A request into the shared finalized Execution Gate, and withholds 202 until a mandatory result receiver commits the Task. The real Messenger Unix client passes cross-repository tests. `tos-ai` `14c7888`, merged as main `526fc6e`, makes that result-before-202 condition durable in a single-owner mode-`0700` outbox: first write is create-exclusive and fsynced, exact pending/complete retries are idempotent, profile/sender/conversation/digest/body substitution fails, deterministic pending recovery survives restart, and completion requires the exact result digest. Messenger `ecf4365`, merged as main `74e7cc2`, adds local request v7 result-only daemon composition; identity, network, clock, schema and Event ID remain daemon-owned, while MCP-call synthesis, cross-profile and text/room smuggling fail closed. `tos-ai` `311bcab`, merged as main `7cd3e48`, drains the outbox through exact sender→pair-session/Endpoint routes, derives stable source+digest idempotency and source-time expiry, and completes only after Messenger returns the queued Event ID. A real Messenger journal/runtime-socket/client test proves canonical queueing and no replay after both journals restart. `tos-ai` `bcf5846`, merged as main `a1126e4`, adds the strict production worker that assembles the profile consumer, quorum finalized-state Gate, single frozen V1 container policy/contract, persistent artifact locator, outbox publisher and hardened systemd unit. Duplicate/unknown configuration authority, socket aliasing, weak quorum and incomplete sender return routing fail closed; full race/vet/build and both remote CI jobs pass. Still pending: selected live inbound transport and independently operated evidence |
| MSG-017 | MCP event bridge | `tos-messenger` / `tos-ai` | 🟡 `tos-ai` owns the MCP execution adapter. Messenger `6e91aed`, merged as main `48072c6`, fixes both MCP kinds to protocol `mcp`, permanently excludes `mcp.call` and `mcp.result` from the general model/runtime inbox, and hands the complete canonical Event v2 to a separate fixed bounded proxy-free owner-private Unix endpoint under kind-specific durable application leases. A2A/MCP socket aliasing and cross-profile delivery fail closed; failure retry and restart-safe completion are tested. `tos-ai` `551b64f`, merged as main `96bf0ef`, adds the paired mode-`0600` Unix service: it independently recomputes canonical Event v2, enforces exact network/sender/conversation policy and carriage version, strictly maps calls/results to the existing MCP adapter and shared finalized Execution Gate, and withholds 202 until a mandatory result receiver commits the Output. The real Messenger Unix client and cross-socket refusal pass cross-repository tests. `tos-ai` `14c7888`, merged as main `526fc6e`, makes that result-before-202 condition durable in a single-owner mode-`0700` outbox: first write is create-exclusive and fsynced, exact pending/complete retries are idempotent, profile/sender/conversation/digest/body substitution fails, deterministic pending recovery survives restart, and completion requires the exact result digest. Messenger `ecf4365`, merged as main `74e7cc2`, adds local request v7 result-only daemon composition; identity, network, clock, schema and Event ID remain daemon-owned, while MCP-call synthesis, cross-profile and text/room smuggling fail closed. `tos-ai` `311bcab`, merged as main `7cd3e48`, drains the outbox through exact sender→pair-session/Endpoint routes, derives stable source+digest idempotency and source-time expiry, and completes only after Messenger returns the queued Event ID. A real Messenger journal/runtime-socket/client test proves canonical queueing and no replay after both journals restart. `tos-ai` `bcf5846`, merged as main `a1126e4`, adds the strict production worker and fixes the production direction boundary: locally executed call outputs alone enter the outbound publisher, while remotely returned `mcp.result` Events commit to a distinct restart-safe terminal journal and can never be echoed back. The same strict config, frozen runtime, systemd and full local/remote gates pass. Still pending: selected live inbound transport and independently operated evidence |
| MSG-018 | Agent Packet carriage and Execution Gate adapter | `tos-messenger` / `tos-service-protocol` / `tos-ai` | 🟡 exact E2EE carriage, finalized verification, durable nonce replay recovery, `tos-ai` Gate adapter, complete three-transport matrix, and a bounded canonical Messenger→owner-private OpenFox provider socket with independent reverification exist. Messenger `cb97f0d` adds admitted-event leasing retained by daemon v9, provider-failure retry, restart-safe dual completion and atomic exclusion from the general model/runtime inbox; selected live inbound transport and independently operated evidence remain pending |
| MSG-019 | Quote/escrow/Receipt reference profile | `tos-messenger` / `tos-service-protocol` | 🟡 typed terms, mandates, budgets, durable negotiation, resolver contract, concrete finalized-chain quote resolver, and a crash-safe one-time commitment→escrow/class ledger implemented. Messenger `0d5988f` keeps the owner-signed digest-only locator path. Messenger `40e06ff`/`dcfca91` assembles the resolver retained by daemon v9 and exposes exact read-only verification of a directly supplied funded escrow without persisting runtime authority; OpenFox `cfa58ee7`/`6ae673bf` maps the complete protocol terms/address and makes verification mandatory after finalized funding and before dispatch, including recovery. Protocol `94d38f8` and OpenFox `755fbf2d`/`8d0bcedf`/`3fae4f91` add the concrete frozen 2-of-3 chain buyer stack, owner-private budget/checkpoint/purchase journals, exact custody-reviewed escrow deployment, Messenger-authorized funding, three-transport verified dispatch and terminal settlement recovery. OpenFox `42268bd3`/`ab356fc1` add the policy-gated autonomous projection onto the same authoritative purchase key/journal; `129ec08d`/`d3a64104` and protocol `284fc3a` harden every autonomous `tosctl` custody call and require provider signer custody; protocol `3809bb7` automates reviewed finalized Capability and immutable manifest publication. Commitment/address/account/provider/network/term/StateInit/message/policy/task/executable/config substitution, ambient-environment injection, redirects, missing authority and bypasses fail closed. Independent buyer/provider settlement and live-node evidence remain missing |
| MSG-020 | Multi-device synchronization | `tos-messenger` | 🟡 succession, revocation, per-pair sessions, fan-out, device-local private generations, fixed-roster public collection, strict device API, config v9 planner/third listener, restart finalization, complete-set replenishment, isolated and externally verified Endpoint signing, expiry/pruning, rollback/equivocation, deterministic durable-generation → immutable HTTPS objects → signed locator → native-DHT scheduling, peer ledger/admission, production DHT/HTTPS refresh, `ba98bc7` stock-command operator-resource assembly, and `b76376e` deterministic two-Descriptor cross-observer fork evidence/stock verification are implemented. `3bf27ae` adds bounded Owner-signed direct-device history export, restart-stable cursor/digest paging, delegation/roster enforcement, idempotent pair-session queueing and daemon-only immutable display import; `17cd7f6` adds bounded Owner-only observation with no application lease. Room history is refused rather than weakening MLS no-past secrecy. Independently operated publication/evidence exchange and live transport/catch-up evidence remain missing |
| MSG-021 | Private Room protocol and MLS comparison | `tos-messenger` / `openfox` | 🟡 Signed room authority/transfer, bounded epoch-bound roles, and auditable queued/applied-history `hide`/`restore` are durable and tested; production admission re-verifies finalized authority and applies moderation before queue publication, while OpenFox persists the presentation overlay, tombstones hidden model/UI history, withdraws action lineage and supports restore. Pinned OpenMLS supplies secrecy/PCS, encrypted OpenFox chat through per-Agent state owners, bounded capacity, and 2-of-2 independently keyed Mailbox offline catch-up across two PCS epochs. Messenger main `f0ee6df` additionally carries a private exact-Commit-bound removal through the runnable opaque Relay, persists the membership boundary before later delivery, and proves restart plus removed-member exclusion. RoomRecord v2 requires resynchronization into fail-closed v3. Authenticated independently operated network Relay evidence, independent review, and second implementation remain open |
| MSG-022 | Public channel Overlay integration | `tos-messenger` / `tos` | 🟡 TOS Overlay exists; Messenger `a2cd605` adds the route-neutral `pkg/publicchannel` candidate with finalized publisher/moderator authority, digest-linked profile succession/fork detection, signed content-addressed Events, causal/per-publisher ordering, immutable moderation projection, convergent history heads and exact missing-ID repair. `e358dfd` adds a single-writer crash-safe local profile/Event/history ledger with immutable-before-pointer ordering, monotonic histories, restart re-verification and rollback/fork/damage refusal. `4eb6c9f` adds deterministic signing/ID/head vectors and five decode/verify adversarial cases. `7c6e43d` adds strict bounded recursive tip/parent fetch, exact response partitioning, retryable unavailability, fetched-authority checks, idempotent merge and full-head reproduction before commit. `5eab660` adds authenticated per-attempt peer/head/fetch/byte/unavailable ceilings and distinct-peer work prioritization without granting consensus authority. `14f01b9` adds the pinned native ADNL Overlay/RLDP carrier, application-authority rechecks, symmetric resource ceilings and real two-Gateway UDP teardown/restart evidence. `beaa358` adds the correct `pub.overlay` key→short-ID derivation, native-DHT address/signed-node publication and discovery, exact ADNL key rebinding, per-peer carrier lifecycle, automatic verified history commit and a runnable strict `tos-public-channeld`; real two-node UDP discovery/sync plus full durable restart run in `make verify`. `a3c5910` adds strict deterministic Storage Bag snapshots, a no-shell bounded CLI publisher, single-writer durable receipts and authority-free BagID hints with isolated replay accounting; native UDP integration proves injected publication/hint propagation and restart no-republish, while a fixture executable verifies the CLI boundary. `e3ea408` adds bounded stock CLI Bag download/catch-up, exact returned status/path binding, strict finalized snapshot verification, durable crash-recoverable locator receipts, alternate-Bag refusal and restart no-redownload. A second real-UDP test proves Sites-only Overlay hint → injected download → verified commit and full node/store/catch-up restart without RLDP history; the hermetic CLI download test remains a process fixture. `961c02d` adds opt-in acceptance across two real same-host daemons and a locally signed DHT, proves publish/download/strict verification/offline receipt replay, and normalizes stock uppercase BagID plus the exact trailing directory slash without relaxing wire canonicality. `c19d759` adds a fail-atomic incremental fetch cursor and repeatable 1/256/1,024/4,096/65,536-Event resource benchmarks. Main `8c26720` raises the native request budget to represent a 65,536-Event linear history, serves exact IDs from an immutable position index, fails closed on index damage, and records 1/8/32-peer same-host strict-path measurements; final complete verification remains mandatory. Independently operated/public-network Storage evidence, representative-device/public-network concurrent-peer calibration, multi-operator failover, independent vector consumption/review and second implementation remain |
...
| MSG-028 | Independent multi-operator deployment | deployments/runbooks | ⬜ no qualifying evidence exists. Completion requires separately controlled operators/sites and, where conformance is claimed, a genuinely independent implementation; another same-host process, repository worktree, key, or Codex run under this operator cannot satisfy independence |
...
local API v10 route-free `messages.send-direct`/`messages.reply-direct`,
daemon-config v10 authenticated Native DNS, the AgentLoop message tool's
high-level `recipient` argument, canonical AgentID-keyed conversations,
daemon-owned session bootstrap/fan-out and AgentID-to-Endpoint verification.
...
MSG-014 remains 🟡 because independent public-network/operator evidence is
absent. Proactive direct send no longer requires an OpenFox route or existing
session; the daemon owns bootstrap and can deliver through the bounded HTTPS
fallback while the final M0-R route decision remains open.
...
per exact Head and one Head per peer are active, pending work is capped at 256,
failure selects one connected same-Head waiter, success clears redundant
waiters, and distinct ready Heads retain independent progress. Full Head
encoding prevents a same-digest/different-tips substitution from suppressing a
canonical candidate. Focused race tests, complete local `make verify`, and all
...
evidence. Messenger main `ecdcedb` additionally requires matching external
database signatures and a pinned CVD root certificate under `--fips-limits`.
Three live ClamAV 1.5.3/current-daily acceptance rounds pass clean, EICAR,
corrupt-signature and corrupt-certificate cases through the exact sandbox.
Messenger PR #20 and the prior PR #19 each passed all fifteen remote jobs, and
OpenFox PR #7 merged after focused race/vet and complete local `make check`.
These are implemented sub-results, but both work packages remain 🟡: the
qualifying externally approved representative hostile-corpus report and existing independent
transport/operator/review gates remain open.

Messenger main `8913b23` closes the local evidence-format and execution gap:
strict signed manifest/report schemas, safe fixed-authority key handling, exact
clean-build/policy/resource commitments, real `OpenForAgent` execution and
adversarial substitution tests are merged after PR #21 passed all fifteen
jobs. The local clean/EICAR run proves only that plumbing. It does not satisfy
the externally approved representative-corpus gate, so MSG-013 and the counts
remain unchanged.
...
Messenger production commit `74e7cc2` and final OpenFox main `aa02f93e`, after
the DNS contact resolver, protocol result composition, reviewed upstream
fallback-chain merge and verifier integration. Alice's durable request
`acceptance-aa02f93e-20260822-v1` bound Event
`msg_e387831755d1aad4ccaeb6e198f2c9ed8081ab8644b0453f0cb90fdab6cc15c5`
before replay. It received exactly one independently produced AgentLoop reply
from Bob (`msg_82e86a102fe79950291fd01792eb48032938121dccc1769b7b63a2c522aaaffa`)
and Carol (`msg_70fe4912aa5996b4a5a955baee5513f2d9ec9f511885900522f6143271497eea`).
After restarting the opaque Relay, all three private MLS proxies and all three
OpenFox AgentLoop processes together, the committed verifier returned
`exact_replay_stable: true`; every complete 123-record transcript stayed
unchanged, all units were `active/running` with `NRestarts=0`, and all five
deployment artifact hashes matched. The mode-`0600` Relay was 152,784 bytes,
its inode and bytes stayed unchanged through replay, and it contained none of
the opening or reply plaintexts. Exact binary hashes, the machine JSON and the
same-host/local-route limitation are in
`tos-messenger/docs/evidence/OPENFOX_GROUP_LATEST_MAIN_2026-08-22.{md,json}` at
Messenger main `a9a1051`. This is fresh runnable/restart evidence for MSG-014,
but it does not change the 🟡 status: selected-route, public-network,
...
Messenger AgentLoop runner, strict cross-transcript and signed-operator
evidence verification, and the independent two-host runbook. The scoped repository implementation and
...
The same audit found no remaining route-independent implementation package
that can be honestly completed by this operator alone. MSG-004, MSG-008 and
MSG-009 are downstream of the qualifying M0-R finding rather than permission
to preselect a transport; MSG-001/003/005/010/011/013/014/016–022/027–029 require
independent review, implementation, operation or real-network evidence; and
MSG-026/031/032 remain Expansion-Gate locked. A local process, key, repository
clone or additional same-operator run is useful regression evidence but cannot
manufacture independence. This is the current stop boundary for local coding,
not a claim that the Messenger has met the Section 29 completion criterion.
