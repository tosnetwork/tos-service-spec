...
Progress snapshot (2026-08-21, audited through `tos-messenger` `beaa358`,
...
Weighted implementation completion is now **about 93% (bounded estimate
91–95%)**, up from the prior approximately 92% audit. The cumulative increase is the
...
the candidate carrier and may later mirror through TOS Sites, but Overlay
...
- a runnable node publishes its separately provisioned ADNL address and signed
  ...
  after full Gateway/node/store restart.
...
Still open are TOS Sites history mirroring, measured limit calibration,
independently operated malicious/unavailable-Relay convergence, independent
vector consumption/review and a second implementation. The commercial
...
its crash-safe ledger, a live native ADNL Overlay/RLDP carrier, and runnable
native-DHT multi-peer node assembly are implemented; TOS Sites mirroring,
measured calibration and independent network evidence remain open.**
...
- public-channel TOS Sites mirroring, measured abuse/resource calibration and
