...
| `atos_bind_identity`, `atos_revoke_identity` | `identity_bindings:write` (Phase 4A) -- admin only, deliberately no ownership/provider-role visibility precondition, same operator/activation-authority-side model as `atos_evaluate_activation`; never obtainable through passkey/Device Authorization self-service |
| `atos_identity_binding_status` | `identity_bindings:read` (Phase 4A) -- admin only, same model as above |
...
- `atos_bind_identity` / `atos_revoke_identity` — **`identity_bindings:write`** (Phase 4A; new scope, explicit-grant-only like `activation:evaluate`/`execution_signers:write`, never a default consumer or provider scope, never obtainable through the passkey/Device Authorization self-service bundle -- `docs/AUTH.md`'s "Human Account Authentication" section explicitly excludes it). Gateway-operator actions establishing which TOS Agent Identity a `principal_id` is bound to (`docs/IMPLEMENTATION_ROADMAP.md` §8.1, `docs/TOS_RPC.md` §10). Deliberately **not** self-service: a principal cannot bind itself to an arbitrary claimed TOS identity merely by being authenticated. `atos_bind_identity` takes `{principal_id, agent_id, idempotency_key}`; `agent_id` MUST already independently resolve through `IdentityService.ResolveAgentIdentity` -- this tool never creates a new TOS Agent Identity from nothing. Rebinding a principal that already has a DIFFERENT current `agent_id` is rejected as `idempotency_conflict` (the underlying RPC's `ALREADY_EXISTS` maps to the same stable code every other conflicting-request case uses; there is no separate `ALREADY_BOUND` code); resubmitting the SAME `agent_id` under a fresh `idempotency_key` is a harmless no-op (`created:false`). `atos_revoke_identity` takes `{principal_id, reason_code, idempotency_key}`; `revoked:false` is a normal result (nothing was bound), not an error. Unlike `atos_evaluate_activation`, `idempotency_key`'s namespace is the target `principal_id` (not the calling admin's own identity), matching `CreatePrincipalBinding`/`RevokePrincipalBinding`'s own frozen idempotency tuple.
- `atos_identity_binding_status` — **`identity_bindings:read`** (Phase 4A); returns the current LOCAL binding projection for `principal_id` (`bound`, `agent_id`, `network`, `binding_ref`, `status`) without mutating anything or re-consulting the authority. Read-only, no mutation.
